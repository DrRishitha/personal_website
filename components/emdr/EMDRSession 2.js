...
export default function EMDRSession() {
    ...
    const emdrAudio = manifest?.emdr?.[lang] ?? {};
    const instrAudioKey = (pi, ii) => `phase${pi}_inst${ii}`;
    const currentInstrAudio = emdrAudio[instrAudioKey(phaseIndex, instructionIndex)] ?? null;
                    ...
                    <div className={styles.introContent}>
                        {currentPhase.instructions.map((inst, i) => (
                            <p key={i} className={styles.instruction}>
                                {inst}
                                {emdrAudio[instrAudioKey(phaseIndex, i)] && (
                                    <AudioKaraoke entry={emdrAudio[instrAudioKey(phaseIndex, i)]} compact />
                                )}
                            </p>
                        ))}
                    ...
                    <div className={styles.guidedContent}>
                        ...
                        {isActive && currentPhase.instructions && (
                            <div className={styles.guidedInstruction}>
                                ...
                                {currentInstrAudio && (
                                    <AudioKaraoke
                                        key={`emdr-${phaseIndex}-${instructionIndex}`}
                                        entry={currentInstrAudio}
                                        autoPlay
                                        compact
                                    />
                                )}
